...
````
fmrsgendata(n, nComp, nCov, coeff, deviance, mixProp, rho, umax, disFamily)
````

where `n` is the sample size, `nComp` is the order of FMRs model, `nCov` is 
the number of regression covariates, `coeff`, `deviance` and `mixProp` are 
the parameters of regression models, i.e. regression coefficients, 
dispersion of errors (sub-distributions) and the mixing proportions, 
...
The function `fmrsmle` in fmrs package provides maximum likelihood 
estimation for the parameters of an FMRs model. The function has the 
following form,

````
fmrsmle(y, x, delta, nComp, disFamily, initCoeff, initDeviance, 
        initmixProp, ...)
````
...
`initDeviance` and `initmixProp` are initial values for EM and NR 
...
prespecified number. The BIC is provided by the function `fmrsvarsel` for 
