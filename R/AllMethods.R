...
#' @import survival
setMethod(f="fmrs.mle", definition=function(y, delta, x, nComp = 2,
    disFamily = "lnorm", initCoeff, initDispersion, initmixProp, lambRidge = 0,
    nIterEM = 2000, nIterNR = 2, conveps = 1e-8, convepsEM = 1e-8,
    convepsNR = 1e-8, porNR = 2, oracle, penVar = FALSE){
    ...
    if(missing(oracle)) oracle = matrix(1, nrow = nCov+1, ncol = nComp)
    if(any(oracle!=0 & oracle!=1))
    stop("oracle must be a matrix of dimention nCov+1 by nComp with
    only 0 and 1 values.")
    ...
    model = "FMR"
    delta = rep(1, n)
    res=.C("FMR_Norm_Surv_EM_MLE", PACKAGE="fmrs", y = as.double(y),
    x = as.double(as.vector(unlist(x))), delta = as.double(delta),
    Lambda.Ridge = as.double(lambRidge), Num.Comp = as.integer(nComp),
    ...
    actset = as.integer(oracle),
    disnorm = as.integer(1),
    Sigpennom = as.double(sigpennom),
    Sigpendenom = as.double(sigpendenom)
    )
    ...
    deni = (((1 / sigma0[k1])* exp((log(y[i]) - mui)/sigma0[k1]))^(delta[i])) *
    ...
    newX = x[,acs[-1,k1]==1]
    if(acs[1,k1]==1){
    res <- survival::survreg(Surv(time = y, event = delta,
    type = c('right')) ~ 1 + newX ,
    weights = W[,k1],
    dist="weibull",
    ...
    toler.chol=conv.eps.em, iter.max=Num.iterationNR, debug=0, outer.max=10),
    parms=NULL, model=FALSE, x=FALSE,
    ...
    type = c('right')) ~ -1 + newX ,
    weights = W[,k1],
    dist="weibull", init=c(beta0[acs[-1,k1]==1,k1]),
    scale=sigma0[k1],
    control=list(maxiter=Num.iterationNR,
    rel.tolerance=conv.eps.em, toler.chol=conv.eps.em,
    iter.max=Num.iterationNR, debug=0, outer.max=10),
    parms=NULL, model=FALSE, x=FALSE,
    y=TRUE, robust=FALSE, score=FALSE)
    new_alpha0[k1] <- 0
    ...
    sumi3 = sumi3 + W[i,k1] * (- delta[i] / sigma0[k1] + ((log(y[i]) - mui) /
    (sigma0[k1] * sigma0[k1]))*(exp( (log(y[i])-mui)/sigma0[k1]) - delta[i]) )
    sumi5 = sumi5 + W[i,k1] * (delta[i] / (sigma0[k1] * sigma0[k1])+
    ((log(y[i]) - mui) / (sigma0[k1] * sigma0[k1] * sigma0[k1])) *
    ...
    diff = sum((new_sigma0 - sigma0)^2) + sum((new_alpha0 - alpha0)^2) +
    ...
    EBIC5 = loglike1 - 0.5 * Num.Comp * Num.Cov * log(Sample.Size) - 0.5 *
    (Num.Comp * Num.Cov) * log(Num.Cov)
    EBIC1 = loglike1 - 0.5 * (Num.Comp * Num.Cov) * log(Sample.Size) -
    (Num.Comp * Num.Cov) * log(Num.Cov)
    ...
    mui = sum(x[i,] * new_beta0[,k1] * acs[-1,k1]) + new_alpha0[k1] * acs[1,k1]
    deni = (((1 / new_sigma0[k1])* exp((log(y[i]) - mui) /
    new_sigma0[k1]))^(delta[i])) * exp(-exp((log(y[i]) - mui) / new_sigma0[k1]))
    if(deni==0)
    deni= 0.000001
    phi[i,k1] = new_pi0[k1] * deni
    sumi = sumi + phi[i,k1]
    ...
    stop("The family of component-wise distributions is not specified
    correctly.")
    }
    ...
    ncov = nCov,
    ncomp = nComp,
    coefficients = array(rbind(res$Intecept.Hat, matrix(res$Coefficient.Hat,
    nrow = nCov, byrow = FALSE)), dim = c(nCov+1, nComp),
    dimnames = list(xnames,comnames)), dispersion = array(res$Dispersion.Hat,
    dim = c(1,nComp),dimnames = list(NULL,comnames)),
    ...
    dim = c(n, nComp), dimnames = list(NULL,comnames)),
    oracleset = array(matrix(c(oracle), nrow=nCov+1, byrow = FALSE),
...
setMethod(f="fmrs.tunsel", definition=function(y,
    ...
    if(missing(cutpoint)){
    cutpoint = 0.1
    }else if(cutpoint<=0 | !is.numeric(cutpoint)){
    stop("The cutpoint must be a positive integer.")
    }
    ...
    ncomp = nComp,
    lambPen = array(res$Opt.Lambda, dim = c(1,nComp), dimnames = c(list(NULL,
    c(paste("Comp", seq_len(nComp), sep = "."))))),
    lambRidge = lambRidge,
    disFamily = disFamily,
    penFamily = penFamily,
    MCPGam = lambMCP,
...
setMethod(f="fmrs.varsel", definition=function(y,
    ...
    activeset <- array(rbind(res$Intecept.Hat,
    matrix(res$Coefficient.Hat, nrow = nCov, byrow = FALSE)),
    dim = c(nCov+1, nComp), dimnames = list(xnames,comnames))
    activeset[abs(activeset)<=cutpoint]<-0
    activeset[abs(activeset)>cutpoint]<-1
    activeset[1,]<-oracle[1,]
