             ...
             double gamma2)
{
    ...
    for(j = 0; j < ncov1; j++)
        moshtagh[j] = lam1 * gamma2 * (gamma2 + 1) / pow(gamma2 +
            fabs(regcoef[j]), 2);
...
extern "C" {
    ...
    )
    {
        ...
        double sigpennom = *mysigpennom;
        double sigpendenom = *mysigpendenom;
        ...
        for(k1 = 0; k1 < NCOMP; k1++){
            for(i = 0; i < NCOV; i++){
                new_beta0[i][k1] = beta0hat[i][k1] = beta0[i][k1] = initbeta[i][k1] =
                    myinitial_beta[NCOV * k1 + i] * acs[i+1][k1];
            }
            new_alpha0[k1] = alpha0hat[k1] = alpha0[k1] = initalpha[k1] =
                myinitial_alpha[k1] * acs[0][k1];
            new_sigma0[k1] = sigma0hat[k1] = sigma0[k1] = initsigma[k1] =
                myinitial_sigma[k1];
        ...
        while((convg1 != 'y') && (niter1 < EM_maxiter)){
            ...
            for(i = 0; i < nsize; i++){
                ...
                for(k1 = 0; k1 < NCOMP;  k1++){
                    ...
                    deni = pow(eps +  dnorm(resp[i] - mui, 0, sigma0[k1], 0), delta[i]) *
                        pow(eps + 1 - pnorm(resp[i] - mui, 0, sigma0[k1], 1, 0),
                            1 - delta[i]) ;
            ...
            for(i = 0; i < nsize; i++){
                for(k1 = 0; k1 < NCOMP;  k1++){
                    ...
                    V[i][k1] = delta[i] * resp[i] + (1- delta[i]) * (mui + sigma0[k1] *
                        Aw[i][k1]);
                }
            }
            ...
            for(i = 0; i < nsize; i++){
                for(k1 = 0; k1 < NCOMP;  k1++){
                    ...
                    sumi3[k1] += W[i][k1] * (delta[i] + (1 - delta[i]) * (Aw[i][k1] *
                        (Aw[i][k1] - w_s[i][k1])))  ;
                }
            }

            for (k1 = 0; k1 < NCOMP; k1++) {
                new_sigma0[k1] = sqrt((sumi5[k1] + sigpennom) / (sumi3[k1] +
                    sigpendenom));
            }
        ...
        for(i = 0; i < nsize; i++){
            ...
            for(k1 = 0; k1 < NCOMP;  k1++){
                ...
                deni = pow(eps +  dnorm(resp[i] - mui, 0, sigma0hat[k1], 0),
                           delta[i]) *  pow(eps + 1 - pnorm(resp[i] - mui, 0,
                                            sigma0hat[k1], 1, 0), 1 - delta[i]) ;
        ...
        *BIC = -2 * loglike1 + NCOMP * NCOV * log(nsize);
        *EBIC5 = -2 * loglike1 + NCOMP * NCOV * log(nsize) + (NCOMP * NCOV) *
        log(NCOV);
        *EBIC1 = -2 * loglike1 + (NCOMP * NCOV) * log(nsize) + 2 * (NCOMP * NCOV) *
            log(NCOV);
        *AIC = -2 * loglike1 + 2 * (NCOMP * NCOV);
        *GCV = (loglike1) / (nsize * pow(1 - NCOMP * NCOV / nsize, 2));
        *GIC = -2 * loglike1 + (NCOMP * NCOV) * log(nsize);
        *MaxEMiter = niter1;
        ...
        for(i = 0; i < nsize; i++){
            ...
            for(k1 = 0; k1 < NCOMP;  k1++){
                ...
                for(j = 0; j < NCOV; j++)
                    mui += myX[nsize * j + i] * beta0hat[j][k1] * acs[j+1][k1];
                mui += alpha0hat[k1]  * acs[0][k1];
                deni = pow(eps +  dnorm(resp[i] - mui, 0, sigma0hat[k1], 0), delta[i])*
                    pow(eps + 1 - pnorm(resp[i] - mui, 0, sigma0hat[k1], 1, 0),
                        1 - delta[i]) ;
                phi[i][k1] = pi0hat[k1] * deni;
                sumi += phi[i][k1];
            }
...
extern "C" {
    ...
    )
    {
        ...
        while((convg1 != 'y') && (niter1 < EM_maxiter)){
            ...
            for(i = 0; i < nsize; i++){
                ...
                for(k1 = 0; k1 < NCOMP;  k1++){
                    ...
                    mui += alpha0[k1] * acs[0][k1];
                    deni = pow(eps +  dnorm(resp[i] - mui, 0, sigma0[k1], 0), delta[i])*
                        pow(eps + 1 - pnorm(resp[i] - mui, 0, sigma0[k1], 1, 0), 1 - delta[i]) ;
                    phi[i][k1] = pi0[k1] * deni;
                    sumi += phi[i][k1];
                }
                for(k1 = 0; k1 < NCOMP;  k1++){
                    ...
                    for(i = 0; i < ONCOV[k1]; i++){
                        for(j = 0; j < ONCOV[k1]; j++){
                            ...
                            if(i != j)
                                ...
                                oneXTWX[i][j] = sumi + En[k1] * vecsigma[j] + ridge1 *
                                    log(nsize);
                        }
                    }

                    if(acs[0][k1] == 1)
                        oneXTWX[0][0] = oneXTWX[0][0] - (En[k1] * vecsigma[0]) - ridge1 *
                            log(nsize);
...

            {
                ...
                for (k1 = 0; k1 < NCOMP; k1++) {
                    for (i = 0; i < nsize; i++) {
                        sumi4[k1] += W[i][k1] * (delta[i] + (1 - delta[i]) * (Aw[i][k1] *
                            (Aw[i][k1] - w_s[i][k1])))  ;
                    }
                }

                for (k1 = 0; k1 < NCOMP; k1++) {
                    new_sigma0[k1] = sqrt((sumi5[k1] + sigpennom) / (sumi4[k1] +
                        sigpendenom));
                }
            }
        ...
        for(k1 = 0; k1 < NCOMP;  k1++){
            ...
            for(j = 0; j < NCOV; j++){
                beta0hat[j][k1] = new_beta0[j][k1];
                if(fabs(beta0hat[j][k1]) <= cutpoint)
                    selection[j][k1] = 0;
                else
                    selection[j][k1] = 1;
            }
        ...
        for(i = 0; i < nsize; i++){
            for(k1 = 0; k1 < NCOMP;  k1++){
                mui = 0.0;
                for(j = 0; j < NCOV; j++)
                    mui += myX[nsize * j + i] * beta0hat[j][k1] * selection[j][k1] *
                        acs[j+1][k1];
                mui += alpha0hat[k1] * acs[0][k1];
                w_s[i][k1] = (resp[i] - mui) / new_sigma0[k1];
                Aw[i][k1] = dnorm(w_s[i][k1], 0, 1, 0) / (eps + (1 - pnorm(w_s[i][k1],
                                                                 0, 1, 1, 0)));
                V[i][k1] = delta[i] * resp[i] + (1 - delta[i]) * (mui + new_sigma0[k1]*
                    Aw[i][k1]);
            }
        }
        ...
        for(i = 0; i < nsize; i++){
            ...
            for(k1 = 0; k1 < NCOMP;  k1++){
                ...
                deni = pow(eps +  dnorm(resp[i] - mui, 0, sigma0hat[k1], 0), delta[i])*
                    pow(eps + 1 - pnorm(resp[i] - mui, 0, sigma0hat[k1], 1, 0),
                        1 - delta[i]) ;
                sat_den = pow(eps +  dnorm(0, 0, sigma0hat[k1], 0), delta[i]) *
                    pow(eps + 1 - pnorm(0, 0, sigma0hat[k1], 1, 0), 1 - delta[i]) ;
                phi[i][k1] = pi0hat[k1] * deni;
                sumi += phi[i][k1];
            }
            loglike1 += log(sumi);
        ...
        *BIC = -2 * loglike1 + SUM1 * log(nsize);
        *EBIC5 = -2 * loglike1 + (SUM1) * log(nsize) + (SUM1) * log(NCOV);
        *EBIC1 = -2 * loglike1 + (SUM1) * log(nsize) + 2 * (SUM1) * log(NCOV);
        *AIC = -2 * loglike1 + 2 * (SUM1);
        *GCV = (loglike1) / (nsize * pow(1 - SUM1 / nsize, 2));
        *GIC = -2 * loglike1 + (SUM1) * log(nsize);
        *MaxEMiter = niter1;
        /*******The E-step of the EM********/
...
extern "C" {
    ...
    )
    {
        ...
        for(k1 = 0; k1 < NCOMP; k1++){
            for(i = 0; i < NCOV; i++){
                new_beta0[i][k1] = beta0[i][k1] = initbeta[i][k1] =
                    myinitial_beta[NCOV * k1 + i] * acs[i+1][k1];
            }
        ...
        for(i = 0; i < nsize; i++){
            ...
            for(k1 = 0; k1 < NCOMP; k1++){
                ...
                deni = pow(eps +  dnorm(resp[i] - mui, 0, initsigma[k1], 0), delta[i])*
                    pow(eps + 1 - pnorm(resp[i] - mui, 0, initsigma[k1], 1, 0),
                        1 - delta[i]);
        ...
        for(k1 = 0; k1 < NCOMP; k1++){
            ...
            for(l1 = 0; l1 < MaxLim; l1++){
                ...
                for(i = 0; i < nsize; i++){
                    ...
                    Aw[i][k1] = dnorm(w_s[i][k1], 0, 1, 0) / (eps + (1 -
                        pnorm(w_s[i][k1], 0, 1, 1, 0)));
                    V[i][k1] = delta[i] * resp[i] + (1 - delta[i]) * (mui +
                        sigma0[k1] * Aw[i][k1]);
                }
                ...
                for(i = 0; i < ONCOV[k1]; i++)
                    for(j = 0; j < (ONCOV[k1] + 1); j++)
                        ...
                        for(j = 0; j < NCOV; j++){
                            if(acs[j+1][k1] == 1){
                                new_beta0[j][k1]  = onesolution1[l++];
                                if(fabs(new_beta0[j][k1]) <= cutpoint)
                                    selection[j][k1] = 0;
                                else
                                    selection[j][k1] = 1;
                        ...
                        for(i = 0; i < nsize; i++){
                            mui = 0.0;
                            for(j = 0; j < NCOV; j++)
                                mui += myX[nsize * j + i] * new_beta0[j][k1]*selection[j][k1]*
                                    acs[j+1][k1];
                            mui += alpha0[k1] * acs[0][k1];
                            deni = pow(eps +  dnorm(resp[i] - mui, 0, sigma0[k1], 0),
                                       delta[i]) *  pow(eps + 1 - pnorm(resp[i] - mui,
                                                        0, sigma0[k1], 1, 0), 1 - delta[i]);
                            loglike1 += W[i][k1] * log(deni);
                            n1[k1] += W[i][k1];
                        }

                        BIC[l1][k1] = loglike1 - 0.5 * (count1[k1][l1]) * log(n1[k1]) -
                            gam * (count1[k1][l1]) * log(ONCOV[k1]);
            ...
            for(i = 0; i < nsize; i++){
                ...
                for(k1 = 0; k1 < NCOMP; k1++){
                    ...
                    for(j = 0; j < NCOV; j++)
                        mui += myX[nsize * j + i] * beta0[j][k1] * acs[j+1][k1];
                    mui += alpha0[k1] * acs[0][k1];
                    deni = pow((1 / sigma0[k1])* exp((resp[i] - mui) / sigma0[k1]),
                               delta[i]) * exp(-exp((resp[i] - mui) / sigma0[k1])) ;
                    //if (isnan(deni))
                    //  cout << "NAN" << "\t";
                    //                    if (deni < eps2)
                    //                        deni = eps2;
                    ...
                    for(j = 0; j < (NCOV + 1); j++){
                        if(acs[j][k1] == 1){
                            ...
                            for(i = 0; i < nsize; i++){
                                ...
                                mui += alpha0[k1] * acs[0][k1];
                                if(j==0){
                                    sumi += W[i][k1] * (acs[j][k1]  / sigma0[k1]) *
                                        (exp((resp[i] - mui) / sigma0[k1]) - delta[i]);
                                }
                                else{
                                    sumi += W[i][k1] * (myX[nsize * (j-1) + i] * acs[j][k1]  /
                                        sigma0[k1]) * (exp((resp[i]-mui) / sigma0[k1]) - delta[i]);
                                }
                            }
                            oneXTWY[j1++] =  sumi;
                        }
                    ...
                    for(i = 0; i < ONCOV[k1]; i++){
                        for(j = i; j < ONCOV[k1]; j++){
                            ...
                            for(l = 0; l < nsize; l++){
                                ...
                                mui += alpha0[k1]  * acs[0][k1] ;
                                sumi += - W[l][k1] * one_X[l][i][k1] * one_X[l][j][k1] /
                                    (sigma0[k1] * sigma0[k1]) * exp((resp[l]-mui) / sigma0[k1]);
                            }
                            if(i == j)
                    ...
                    for(i = 0; i < ONCOV[k1]; i++)
                        for(j = 0; j < (ONCOV[k1] + 1); j++)
                            ...
                            do {
                                l = 0;
                                if(acs[0][k1] == 1)
                                    new_alpha0[k1] = pow(0.5, alp) * onesolution1[l++] +
                                        alpha0[k1];
                                for(j = 0; j < NCOV; j++){
                                    if(acs[j+1][k1] == 1){
                                        new_beta0[j][k1] = pow(0.5, alp) * onesolution1[l++] +
                                            beta0[j][k1];
                                    }
                                }
                                ...
                                for(i = 0; i < nsize; i++){
                                    ...
                                    mui += new_alpha0[k1] * acs[0][k1];
                                    sumi3[k1] += W[i][k1] * (- delta[i] / sigma0[k1] +
                                        ((resp[i] - mui) / (sigma0[k1] * sigma0[k1])) *
                                        ( exp( (resp[i] - mui) / sigma0[k1]) - delta[i])   );
                                    sumi5[k1] += W[i][k1] * ( delta[i] / (sigma0[k1] *
                                        sigma0[k1]) +  ((resp[i] - mui) / (sigma0[k1] *
                                        sigma0[k1] * sigma0[k1])) * (2 * delta[i] -
                                        (2 + (resp[i] - mui) / sigma0[k1]) * exp( (resp[i] - mui) /
                                            sigma0[k1])  )) ;
                                }
                                sumi5[k1] += sigpennom;
                                sumi3[k1] += sigpendenom;
                                new_sigma0[k1] = sigma0[k1] - pow(0.5, alp) * (1 / sumi5[k1]) *
                                    sumi3[k1];
                                //  new_sigma0[k1] = (new_sigma0[k1] < 0.1)?0.5:new_sigma0[k1];
                                // if (k1 == 0)
                                // new_sigma0[k1] = (new_sigma0[k1] > 10)?2:(new_sigma0[k1]);
                                //                        else
                                // newsigma[k1] = (new_sigma0[k1] > 10)?2:(new_sigma0[k1]);
                                ...
                                for(i = 0; i < nsize; i++){
                                    ...
                                    for(j = 0; j < NCOV; j++)
                                        mui += myX[nsize * j + i] * new_beta0[j][k1]*acs[j+1][k1];
                                    mui += new_alpha0[k1] * acs[0][k1];
                                    deni = pow((1 / new_sigma0[k1]) * exp((resp[i] - mui) /
                                        new_sigma0[k1]), delta[i]) * exp(- exp((resp[i] - mui) /
                                            new_sigma0[k1])) ;
                                    //                            if (deni < eps2)
                                    //                                deni = eps2;
        ...
        for(i = 0; i < nsize; i++){
            ...
            for(k1 = 0; k1 < NCOMP; k1++){
                ...
                for(j = 0; j < NCOV; j++)
                    mui += myX[nsize * j + i] * beta0hat[j][k1] * acs[j+1][k1];
                mui += alpha0hat[k1] * acs[0][k1];
                deni = pow((1 / sigma0hat[k1]) * exp((resp[i] - mui) / sigma0hat[k1]),
                           delta[i]) * exp(- exp((resp[i] - mui) / sigma0hat[k1]));
                phi[i][k1] = pi0hat[k1] * deni;
                sumi += phi[i][k1];
            }
            loglike1 += log(sumi);
...
extern "C" {
    ...
    )
    {
                    ...
                    for(j = 0; j < (NCOV + 1); j++)
                        ...
                        for(i = 0; i < (NCOV + 1); i++){
                            for(j = i; j < (NCOV + 1); j++){
                                ...
                                for(l = 0; l < nsize; l++){
                                    ...
                                    mui += alpha0[k1];
                                    sumi += - W[l][k1] * one_X[l][i] * one_X[l][j] / (sigma0[k1]*
                                        sigma0[k1]) * exp((resp[l] - mui) / sigma0[k1]);
                                }
                                if(i == j)
                                    oneXTWX[i][j] = sumi - En[k1] * vecsigma[j] + ridge1 *
                                        log(nsize);
                        ...
                        for(j = 0; j < (NCOV + 1); j++){
                            ...
                            for(i = 0; i < nsize; i++){
                                ...
                                mui += alpha0[k1];
                                sumi += W[i][k1] * (one_X[i][j] / sigma0[k1]) *
                                    (exp((resp[i] - mui) / sigma0[k1]) - delta[i]);
                            }
                            if (j == 0)
                        ...
                        for(i = 0; i < (NCOV + 1); i++)
                            for(j = 0; j < (NCOV + 2); j++)
                                ...
                                /************************************************************/
                                /*Solving the system Ax=y to get betahat in k-th component*/
                                /************************************************************/
                                ...
                                do {
                                    for(j = 0; j < (NCOV + 1); j++)
                                        if(j == 0)
                                            new_alpha0[k1] = pow(0.5, alp) * onesolution1[j] +
                                                alpha0[k1];
                                        else
                                            new_beta0[j-1][k1] = pow(0.5, alp) * onesolution1[j] +
                                                beta0[j-1][k1];
                                        ...
                                        for(i = 0; i < nsize; i++){
                                            ...
                                            sumi5[k1] += W[i][k1] * ( delta[i] / (sigma0[k1] *
                                                sigma0[k1]) +  ((resp[i] - mui) / (sigma0[k1] *
                                                sigma0[k1] * sigma0[k1])) * (2 * delta[i] -
                                                (2 + (resp[i] - mui) / sigma0[k1]) * exp(
                                                        (resp[i] - mui) / sigma0[k1])  )) ;
                                        }
                                        sumi5[k1]+=sigpennom;
                                        sumi3[k1]+=sigpendenom;
                                        new_sigma0[k1] = sigma0[k1] - pow(0.5, alp) * (1 /
                                            sumi5[k1]) * sumi3[k1];
                                        ...
                                        for(i = 0; i < nsize; i++){
                                            ...
                                            mui += new_alpha0[k1];
                                            deni = pow((1 / new_sigma0[k1]) * exp((resp[i] - mui) /
                                                new_sigma0[k1]), delta[i]) * exp(- exp((resp[i] -
                                                    mui) / new_sigma0[k1])) ;
                                            //                        if (deni < eps2)
                                            //                            deni = eps2;
                                            newloglike1 += W[i][k1] * log(deni);
        ...
        //                mui += alpha0hat[k1];
        //    sumi3[k1] += W[i][k1] * (- 1 / sigma0[k1] + ((resp[i] - mui) /
        //(sigma0[k1] * new_sigma0[k1])) * ( exp( (resp[i] - mui) /
        //sigma0[k1]) - delta[i])   );
        //                sumi5[k1] += W[i][k1] * ( 1 / (sigma0[k1] *
        //sigma0[k1]) +  ((resp[i] - mui) / (sigma0[k1] * sigma0[k1] *
        //sigma0[k1])) * (2 * delta[i] - (2 + (resp[i] - mui) / sigma0[k1]) *
        //exp( (resp[i] - mui) / sigma0[k1])  )) ;
        //            }
        //            //sumi5[k1] += sigpennom;
        //            //sumi3[k1] += sigpendenom;
        ...
        for(i = 0; i < nsize; i++){
            ...
            for(k1 = 0; k1 < NCOMP; k1++){
                ...
                //                if (deni < eps2)
                //                    deni = eps2;
                sat_den = pow((1 / sigma0hat[k1]) * exp((resp[i] - 0.0) /
                    sigma0hat[k1]), delta[i]) * exp(- exp((resp[i] - 0.0) /
                        sigma0hat[k1])) ;
                //                if (sat_den < eps2)
                //                    sat_den = eps2;
                phi[i][k1] = pi0hat[k1] * deni;
    ...
    )
    {
        ...
        for(k1 = 0; k1 < NCOMP; k1++){
            ...
            for(l1 = 0; l1 < MaxLim; l1++){
                        ...
                        for(i = 0; i < ONCOV[k1]; i++){
                            for(j = i; j < ONCOV[k1]; j++){
                                ...
                                for(l = 0; l < nsize; l++){
                                    ...
                                    mui += alpha0[k1] * acs[0][k1];
                                    sumi += - W[l][k1] * one_X[l][i][k1] * one_X[l][j][k1] /
                                        (sigma0[k1] * sigma0[k1]) * exp((resp[l] - mui) /
                                            sigma0[k1]);
                                }
                                if(i == j)
                                    oneXTWX[i][j] = sumi - En[k1] * vecsigma[j] + ridge1 *
                            ...
                            if(acs[j][k1] == 1){
                                ...
                                for(i = 0; i < nsize; i++){
                                    ...
                                    if(j == 0){
                                        ...
                                        sumi += W[i][k1] * (myX[nsize * (j-1) + i] * acs[j][k1] /
                                            sigma0[k1]) * (exp((resp[i] - mui) / sigma0[k1]) -
                                                delta[i]);
                                    }
                                }
                                if( j != 0){
                        ...
                        do {
                            ...
                            for(i = 0; i < nsize; i++){
                                ...
                                for(l = 0; l < NCOV; l++)
                                    mui += myX[nsize * l + i] * new_beta0[l][k1] * acs[l+1][k1];
                                mui += new_alpha0[k1] * acs[0][k1];
                                sumi3[k1] += W[i][k1] * (- delta[i] / sigma0[k1] + ((resp[i] -
                                    mui) / (sigma0[k1] * sigma0[k1])) * ( exp( (resp[i] - mui) /
                                        sigma0[k1]) - delta[i])   );
                                sumi5[k1] += W[i][k1] * ( delta[i] / (sigma0[k1] * sigma0[k1])+
                                    ((resp[i] - mui) / (sigma0[k1] * sigma0[k1] * sigma0[k1])) *
                                    (2 * delta[i] - (2 + (resp[i] - mui) / sigma0[k1]) *
                                    exp( (resp[i] - mui) / sigma0[k1])  )) ;
                            }
                            new_sigma0[k1] = sigma0[k1] - pow(0.5, alp) * (1 / sumi5[k1]) *
                                sumi3[k1];
                ...
                BIC[l1][k1] = loglike1 - 0.5 * (count1[k1][l1]) * log(n1[k1]) - gam *
                    (count1[k1][l1]) * log(ONCOV[k1]);
